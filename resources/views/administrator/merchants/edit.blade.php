...
@section('content')
    ...
    <div class="container-fluid">
        ...
        <div class="row">
            <div class="col-12">
                <div class="page-title-box">
                    ...
                    <h4 class="page-title">Edit Workshop</h4>
                </div>
            </div>
        </div>
        ...
        <div class="row">
            <div class="col-12">
                <form id="merchantForm" method="POST" action="{{ route('administrator.merchants.update', $merchant->id) }}"
                            ...
                            <div class="row">
                                ...
                                <div class="col-sm-6 mb-2 {{ $errors->has('name') ? 'has-error' : '' }}">
                                    ...
                                    <input type="text" class="form-control" id="name" name="name"
                                        placeholder="Enter Workshop Name" value="{{ old('name', $merchant->name) }}">
                                    @error('name')
                                ...
                                <div class="col-sm-6 mb-2 {{ $errors->has('status') ? 'has-error' : '' }}">
                                    ...
                                    <select name="status" id="statuses" class="form-select form-control-sm">
                                        ...
                                        <option value="active"
                                            {{ old('status', $merchant->status) == 'active' ? 'selected' : '' }}>Active
                                        </option>
                                        <option value="blocked"
                                            {{ old('status', $merchant->status) == 'blocked' ? 'selected' : '' }}>Blocked
                                        </option>
                                        <option value="suspended"
                                            {{ old('status', $merchant->status) == 'suspended' ? 'selected' : '' }}>
                                            Suspended
                                        </option>
                                    </select>
                                    @error('status')
                                ...
                                <div class="col-sm-6 mb-2 {{ $errors->has('email') ? 'has-error' : '' }}">
                                    ...
                                    <input type="email" class="form-control" id="email" name="email"
                                        placeholder="Enter login email" value="{{ old('email', $merchant->email) }}">
                                    @error('email')
                                ...
                                <div class="col-sm-6 mb-2 {{ $errors->has('password') ? 'has-error' : '' }}">
                                    ...
                                    <span class="helper-text text-muted"><small>Enter only to change password</small></span>
                                </div>
                                @if($merchant->id != 30)
                                <div class="col-sm-12 mb-2">
                                    ...
                                    <select id="invoice_frequency" class="form-select" name="invoice_frequency">
                                        ...
                                        <option value="weekly"
                                            {{ old('invoice_frequency', $merchant->invoice_frequency) == 'weekly' ? 'selected' : '' }}>
                                            Weekly</option>
                                        <option value="monthly"
                                            {{ old('invoice_frequency', $merchant->invoice_frequency) == 'monthly' ? 'selected' : '' }}>
                                            Monthly
                                        </option>
                                        <option value="quarterly"
                                            {{ old('invoice_frequency', $merchant->invoice_frequency) == 'quarterly' ? 'selected' : '' }}>
                                            Quarterly
                                        </option>
                                        <option value="annually"
                                            {{ old('invoice_frequency', $merchant->invoice_frequency) == 'annually' ? 'selected' : '' }}>
                                            Annually
                                        </option>
                                    </select>
                                ...
                                @else
                                <input type="hidden" name="invoice_frequency" value="weekly">
                                @endif
                            </div>
                        </div>
                    </div>
                            ...
                            <div class="row">
                                ...
                                <div class="col-sm-6 mb-2 {{ $errors->has('store_contact_name') ? 'has-error' : '' }}">
                                    ...
                                    <input type="text" class="form-control" id="store_contact_name"
                                        ...
                                        value="{{ isset($merchant->storeDetail) ? $merchant->storeDetail->name : old('store_contact_name') }}">
                                ...
                                <div class="col-sm-6 mb-2 {{ $errors->has('store_contact_email') ? 'has-error' : '' }}">
                                    ...
                                    <input type="email" class="form-control" id="store_contact_email"
                                        ...
                                        value="{{ isset($merchant->storeDetail) ? $merchant->storeDetail->email : old('store_contact_email') }}">
                                ...
                                <div class="col-sm-6 mb-2 {{ $errors->has('phone') ? 'has-error' : '' }}">
                                    ...
                                    <input type="text" class="form-control" id="phone" name="phone"
                                        ...
                                        value="{{ isset($merchant->storeDetail) ? $merchant->storeDetail->phone : old('phone') }}">
                                    @error('phone')
                                        <span id="name-error" class="error invalid-feedback">{{ $message }}</span>
                                    @enderror
                                ...
                                <div class="col-md-6 mb-2 {{ $errors->has('address') ? 'has-error' : '' }}">
                                    ...
                                    <div class="input-group input-group-merge">
                                        ...
                                        <input id="latitude" type="hidden" class="form-control" name="latitude"
                                            value="{{ isset($merchant->storeDetail) ? $merchant->storeDetail->latitude : old('latitude') }}">
                                        <input id="longitude" type="hidden" class="form-control" name="longitude"
                                            value="{{ isset($merchant->storeDetail) ? $merchant->storeDetail->longitude : old('longitude') }}">
                                        <input id="address" type="text"
                                            class="form-control @error('address') is-invalid @enderror" name="address"
                                            value="{{ isset($merchant->storeDetail) ? $merchant->storeDetail->address : old('address') }}"
                                            autocomplete="off" placeholder="Store Address">
                                    </div>
                                    @error('address')
                                ...
                                <div class="col-sm-12 mb-2 {{ $errors->has('building_floor_room') ? 'has-error' : '' }}">
                                    ...
                                    <input type="text" class="form-control" id="building_floor_room"
                                        ...
                                        value="{{ isset($merchant->storeDetail) ? $merchant->storeDetail->building_floor_room : old('building_floor_room') }}">
                                ...
                                <div class="col-sm-12 mb-2 {{ $errors->has('notes') ? 'has-error' : '' }}">
                                    ...
                                    <textarea class="form-control" id="notes" name="notes" placeholder="Instruction or notes" rows="3">{{ isset($merchant->storeDetail) ? $merchant->storeDetail->notes : old('notes') }}</textarea>
                                    @error('notes')
                            ...
                            <div class="row">
                                ...
                                <div class="col-sm-6 mb-2 {{ $errors->has('billing_name') ? 'has-error' : '' }}">
                                    ...
                                    <input type="text" class="form-control" id="billing_name" name="billing_name"
                                        ...
                                        value="{{ isset($merchant->billingDetail) ? $merchant->billingDetail->name : old('billing_name') }}">
                                    @error('billing_name')
                                ...
                                <div class="col-sm-6 mb-2 {{ $errors->has('billing_phone') ? 'has-error' : '' }}">
                                    ...
                                    <input type="text" class="form-control" id="billing_phone" name="billing_phone"
                                        ...
                                        value="{{ isset($merchant->billingDetail) ? $merchant->billingDetail->phone : old('billing_phone') }}">
                                    @error('billing_phone')
                                ...
                                <div class="col-sm-6 mb-2 {{ $errors->has('billing_email') ? 'has-error' : '' }}">
                                    ...
                                    <input type="email" class="form-control" id="billing_email" name="billing_email"
                                        ...
                                        value="{{ isset($merchant->billingDetail) ? $merchant->billingDetail->email : old('billing_email') }}">
                                    @error('billing_email')
                                ...
                                <div class="col-md-6 mb-2 {{ $errors->has('billing_address') ? 'has-error' : '' }}">
                                    ...
                                    <div class="input-group input-group-merge">
                                        ...
                                        <input id="billing_latitude" type="hidden" class="form-control"
                                            ...
                                            value="{{ isset($merchant->billingDetail) ? $merchant->billingDetail->latitude : old('billing_latitude') }}">
                                        <input id="billing_longitude" type="hidden" class="form-control"
                                            name="billing_longitude"
                                            value="{{ isset($merchant->billingDetail) ? $merchant->billingDetail->longitude : old('billing_longitude') }}">
                                        <input id="billing_address" type="text"
                                            class="form-control @error('billing_address') is-invalid @enderror"
                                            name="billing_address"
                                            value="{{ isset($merchant->billingDetail) ? $merchant->billingDetail->address : old('billing_address') }}"
                            ...
                            <div class="row">
                                ...
                                <div class="col-sm-12 mb-2">
                                   ...
                                   {{-- @if( count($services) > 0)
                                    ...
                                    <div class="col-md-3">
                                    <input type="checkbox"
                                    {{ in_array($merchant->id, old('services', $service->merchants)) ? 'checked' : '' }}
                                    ...
                                    <select name="services[]" id="service" class="form-select form-control-sm select2"
                                        ...
                                        @foreach ($services as $service)
                                            <option value="{{ $service->id }}"
                                                {{ in_array($merchant->id, old('services', $service->merchants)) ? 'selected' : '' }}>
                                                {{ $service->name }}</option>
                                        @endforeach
                                    </select>
...
@push('scripts')
    ...
    <script>
        ...
        var iti = window.intlTelInput(input, {
            initialCountry: "{{ isset($merchant->storeDetail) ? $merchant->storeDetail->iso2 : old('iso2') }}",
